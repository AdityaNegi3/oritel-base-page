...
type UseThemeSyncReturn = [ThemeMode, React.Dispatch<React.SetStateAction<ThemeMode>>];

const getRootTheme = (): ThemeMode => "light";

const useThemeSync = (): UseThemeSyncReturn => {
  const [theme] = useState<ThemeMode>("light");
  const noopDispatch = (() => {
    // no-op (dark theme removed)
  }) as React.Dispatch<React.SetStateAction<ThemeMode>>;
  return [theme, noopDispatch];
};

const AnimatedGlyph = ({ variant = "signal", theme = "dark" }: AnimatedGlyphProps) => {
  const stroke = theme === "dark" ? "#f5f5f5" : "#111111";
  const fill =
    theme === "dark" ? "rgba(255,255,255,0.12)" : "rgba(17,17,17,0.08)";
...
export function HeroMonochromeLaunch() {
  ...
  const clearSpotlight = (event: React.MouseEvent<HTMLDivElement>) => {
    ...
    <section
        ref={sectionRef}
        className={`relative mx-auto flex w-full max-w-6xl flex-col gap-16 px-6 pb-16 pt-20 transition-opacity duration-700 md:gap-20 lg:px-12 ${
          visible
            ? "motion-safe:animate-[hero1-intro_1s_cubic-bezier(.25,.9,.3,1)_forwards]"
            : "opacity-0"
        }`}
      >
        <header className="flex flex-col items-center gap-10 text-center lg:flex-row lg:items-center lg:justify-between lg:gap-16 lg:text-left">
          {/* Left: written part */}
          <div className="flex w-full flex-col items-center gap-10 text-center lg:w-1/2 lg:items-start lg:text-left">
            <div className="flex items-center gap-4 text-xs uppercase tracking-[0.4em] lg:justify-start">
              <span className={`rounded-full border px-4 py-1 ${palette.border} ${palette.accent}`}>
                ORITEL
              </span>
              <span className={palette.subtle}>CENTER OF ALL OPERATIONS</span>
            </div>
            ...
            <div className="space-y-4 max-w-2xl">
              ...
              <p className={`text-lg md:text-xl ${palette.subtle}`}>
                CENTER OF ALL OPERATIONS
              </p>
              <p className="text-base text-neutral-400">powered by osiris marketing</p>
            </div>
          ...
          {/* Right: image */}
          <div className="relative w-full overflow-hidden rounded-3xl border backdrop-blur-xl mt-6 lg:mt-2 lg:w-1/2">
            ...
            <figure className="relative flex w-full flex-col">
              <div className="relative w-full overflow-hidden">
                ...
                />
                <span className="pointer-events-none absolute inset-0 bg-gradient-to-b from-black/50 via-transparent to-black/30 mix-blend-soft-light" />
              ...
              <figcaption className={`flex items-center justify-between px-6 py-4 text-xs uppercase tracking-[0.35em] ${palette.subtle}`}>
                <span>Dashboard</span>
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
          {[
            ...
            <div
              ...
              className={`group relative overflow-hidden rounded-2xl border px-6 py-5 text-sm uppercase tracking-[0.3em] transition duration-500 ${palette.border} ${palette.highlight}`}
            >
              <span className={`block text-xs ${palette.subtle}`}>Capability</span>
              <span className="mt-4 block text-base font-semibold tracking-[0.2em]">
                {label}
              </span>
              <span
